...
while {_altCheck} do {
	...
	if (isEngineOn _heli) then {
		...
		// on the deck - this calculates deployment orders for AI troops just dropped off 
		if ((_atl1) < 2.5) then {
			...
			if (_speed < 1) then {
				...
				if (_speed < 1) then {
					...
					_extractMarker setMarkerAlpha 0.5;
					...
					// new - now check if the tinman bool is on or off - if off, send them in direct, if on, disembark and have them go nearby 
					if (BLUMAN) then {
						// get out and wait 
						systemchat "WAITING FOR ORDERS";
					} else {
						// all go in 
						_movePos = getMarkerPos 'REDZONE';
						_units = allUnits inAreaArray _markerName;
